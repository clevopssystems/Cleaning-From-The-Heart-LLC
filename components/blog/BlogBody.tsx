  ...
  return (
    <figure>
      <Image src={image.src} alt={image.alt} width={image.width} height={image.height} priority={priority}
        sizes="(min-width: 1024px) 760px, 100vw" className="h-auto w-full rounded-2xl" />
    ...
    switch (block.type) {
      ...
      case "quote": return (
        <blockquote key={index} className="border-l-2 border-accent pl-6">
          ...
          {block.attribution ? <p className="text-sm text-muted">{block.attribution}</p> : null}
        </blockquote>
      );
      case "callout": return (
        <aside key={index} className="rounded-xl border border-brand-100 bg-surface p-5 sm:p-6">
          {block.title ? <p className="mb-2 font-semibold">{block.title}</p> : null}
          <p><InlineText text={block.text} /></p>
        </aside>
      );
      case "table": return (
        <div key={index} role="region" aria-label={block.caption} tabIndex={0} className="max-w-full overflow-x-auto rounded-xl border border-brand-100 p-4">
          <table>
            <caption>{block.caption}</caption>
            <thead className="bg-surface"><tr>{block.columns.map((column, columnIndex) => <th key={columnIndex} scope="col">{column}</th>)}</tr></thead>
            <tbody>{block.rows.map((row, rowIndex) => <tr key={rowIndex} className="border-t border-brand-100">{row.map((cell, cellIndex) => <td key={cellIndex}><InlineText text={cell} /></td>)}</tr>)}</tbody>
          </table>
        </div>
      );
      case "faq": return (
        <section key={index} aria-label={block.title}>
          ...
          <div className="mt-5 space-y-3">
            {block.items.map(item => (
              <details key={item.question} className="rounded-2xl border border-brand-100">
                <summary className="cursor-pointer px-5 py-4 text-base font-semibold">{item.question}</summary>
