import { ArrowRight } from "lucide-react";
import Link from "next/link";
import { Container } from "@/components/shared/Container";
import { siteConfig } from "@/lib/site";

export function HeroSection() {
  return (
    <section className="relative -mt-16 overflow-hidden bg-ink text-white">

      {/*
        BACKGROUND VIDEO
        Files needed in /public/home/hero/:
          hero-bg.mp4     , H.264, 1920×1080, no audio, target < 2 MB
          hero-bg.webm    , VP9/AV1 encode of the same clip (~40% smaller than MP4)
          hero-bg-poster.jpg, first frame, ~30 KB JPEG
      */}
      <video
        autoPlay
        muted
        loop
        playsInline
        preload="auto"
        className="hero-video absolute inset-0 h-full w-full object-cover"
        aria-hidden
      >
        <source src="/home/hero/hero-bg.mp4" type="video/mp4" />
      </video>

      {/* Dark overlay */}
      <div className="absolute inset-0 bg-ink/65" aria-hidden />

      {/* Brand gradient tint */}
      <div className="absolute inset-0 bg-gradient-to-br from-brand-900/50 to-transparent" aria-hidden />

      {/* Grid texture */}
      <div className="absolute inset-0 opacity-10 surface-grid" aria-hidden />

      {/* Decorative glow */}
      <div className="absolute -left-32 -top-32 h-96 w-96 rounded-full bg-brand-600/20 blur-3xl" aria-hidden />

      <Container className="relative section-shell">
        <div className="max-w-3xl">
          <div>
            {/* Credibility badge */}
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-accent/30 bg-accent/10 px-3.5 py-1.5">
              <span className="text-xs font-semibold text-accent">Serving Seattle &amp; surrounding areas</span>
            </div>

            <p className="eyebrow-light">Seattle&apos;s Trusted Cleaning Company</p>
            <h1 className="text-balance text-5xl font-bold leading-[1.08] tracking-tight text-white md:text-6xl lg:text-[4.25rem]">
              Seattle Commercial &amp; Residential Cleaning Backed by 30+ Years of Experience
            </h1>
            <p className="mt-6 max-w-xl text-base leading-relaxed text-white/70 md:text-lg">
              Professional cleaning, floor care, window cleaning, pressure washing, and move-in/move-out services
              for homes, offices, schools, property managers, and commercial spaces across the Seattle area.
            </p>

            {/* Proof points, plain, no bullet markers */}
            <p className="mt-6 text-sm text-white/70">
              30+ years of cleaning experience · Commercial &amp; residential · Seattle-area service
            </p>

            {/* CTAs */}
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              <div className="flex flex-col items-start gap-1">
                <Link href="/contact#quote-form" className="cta-gold w-full sm:w-auto">
                  {siteConfig.primaryCta}
                </Link>
                <span className="pl-1 text-[11px] text-white/45">Free quote · No obligation</span>
              </div>
              <div className="flex flex-col items-start gap-1">
                <Link href="#work-gallery" className="cta-ghost w-full sm:w-auto">
                  <ArrowRight className="mr-2 h-4 w-4" aria-hidden />
                  View Our Work
                </Link>
                <span className="pl-1 text-[11px] text-white/45">Real photos from real jobs</span>
              </div>
            </div>

          </div>
        </div>
      </Container>

      <div className="border-t border-white/10 bg-black/20">
        <Container className="flex flex-wrap items-center justify-between gap-3 py-3.5 text-xs text-white/60 sm:text-sm">
          <span className="font-semibold text-white">{siteConfig.name}</span>
          <span>Seattle, Renton, Bellevue, Kent, Everett &amp; More</span>
          <span>{siteConfig.businessHours}</span>
        </Container>
      </div>
    </section>
  );
}
